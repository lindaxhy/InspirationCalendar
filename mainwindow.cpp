    ...
    , ui(new Ui::MainWindow)
{
    ...
    m_aiService->setApiKey("...");
    connect(m_aiService, &AIService::analysisReady, this, &MainWindow::handleAiAnalysis);
    connect(m_aiService, &AIService::errorOccurred, this, &MainWindow::handleAiError);
}
...
void MainWindow::loadStyleSheet(const QString &fileName)
{
    ...
    if(file.open(QFile::ReadOnly | QFile::Text))
    {
        ...
        if (currentFontName != "")
            style.replace("font-family: \"Microsoft YaHei\";", QString("font-family: \"%1\";").arg(currentFontName));
        qApp->setStyleSheet(style);
        file.close();
    }
}

void MainWindow::setColorMode(const QString &mode)
{
    if(mode == "black")
    {
        loadStyleSheet("style3.qss");
    }
    else if(mode == "white")
    {
        loadStyleSheet("style2.qss");
    }
}
...
void MainWindow::setFontName(const QString &fontName) {
...
void MainWindow::onInspirationClicked() {
    ...
    QFile imgTest(fullImagePath);
    qDebug() << "Exists?" << imgTest.exists();
...
void MainWindow::onHistoryEventClicked() {
    ...
    QString question = "如果ESA从未成立，2025年的太空格局会怎样？";
    // 2. 显示三个“平行时空猜想”选项
    QMessageBox box;
    ...
    if (m_aiService) {
        m_aiService->generateHistoricalAnalysis(eventTitle, question, userChoice, this);
    } else {
        // 4. 展示AI分析
...
void MainWindow::handleAiAnalysis(const QJsonObject &result)
{
    // facts 是数组
    QStringList factList;
    QJsonArray factsArray = result["facts"].toArray();
    for (const QJsonValue &fact : factsArray) {
        factList.append("- " + fact.toString());
    }
    QString facts = factList.join("\n");

    // analysis 是对象
    QJsonObject analysisObj = result["analysis"].toObject();
    QString shortTerm = analysisObj["short_term"].toString();
    QString longTerm = analysisObj["long_term"].toString();
    QString analysis = QString("短期影响:\n%1\n\n长期影响:\n%2").arg(shortTerm, longTerm);

    // longterm 是 analysis 对象下的 long_term，不是顶层字段，已处理

    // interaction（用户问题）
    QJsonObject interactionObj = result["interaction"].toObject();
    QString followUp = interactionObj["follow_up"].toString();
    QString predictionTest = interactionObj["prediction_test"].toString();
    QString userQuestion = QString("后续提问:\n%1\n\n预测测试:\n%2").arg(followUp, predictionTest);

    // 显示
    QString displayText = QString("事实背景:\n%1\n\n分析结果:\n%2\n\n用户问题:\n%3")
                              .arg(facts)
                              .arg(analysis)
                              .arg(userQuestion);

    QMessageBox::information(this, "AI分析结果", displayText);
}

void MainWindow::handleAiError(const QString &errorMessage)
{
    QMessageBox::critical(this, "DeepSeek API错误", errorMessage);
}
